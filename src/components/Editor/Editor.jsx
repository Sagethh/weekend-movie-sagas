...
function Editor(movie) {
    ...
    const [movie2, setMovie2] = useState([]); // main movie array that will be send back to the server
    ...
    const submit = () => { // submit onClick function
        ...
        movie2.push({ title: movieTitle, description: movieDescription, poster: moviePoster, genre_id: movieGenre }); // pushes movie data into movie array to send back to server
    ...
    return ( // main return, what will be shown on the DOM
        <>
          <div>
                <button onClick={test}>Test Editor</button>
            </div>
            <Modal aria-labelledby="Edit Movie Modal" aria-describedby="Upload a movie" className={modal.modal} open={open} onClose={handleClose} closeAfterTransition BackdropComponent={Backdrop} BackdropProps={{timeout: 500}}>
                <Fade in={open}>
                    <div className={modal.paper}>
                        <TextField id="Edit Movie Title" label="Movie Title" variant="outlined" value={editor.movie.title} onChange={(event) => setMovieTitle(event.target.value)}/><br /><br />
                        <TextField id="Edit Movie Description" label="Movie Description" style={{width: "100%"}} multiline maxRows={6} variant="outlined" value={editor.movie.description} onChange={(event) => setMovieDescription(event.target.value)}/><br /><br />
                        <TextField id="Edit Movie Poster URL" label="Movie Poster URL" variant="outlined" value={editor.movie.poster} onChange={(event) => setMoviePoster(event.target.value)}/><br /><br />
                        <TextField select id="Edit Movie Genre Selector" label="Genre" style={{width: "100%"}} SelectProps={{native: true}} variant="outlined" onChange={(event) => setMovieGenre(event.target.value)}>
